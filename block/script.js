...
function draw() {
    ...
    // Draw Miyacchi on top of the paddle
    if (miyacchiImg.complete) {
        const charHeight = state.paddle.width * 0.7;
        const charWidth = charHeight * (miyacchiImg.width / miyacchiImg.height);
        const charX = state.paddle.x + (state.paddle.width - charWidth) / 2;
        const charY = state.paddle.y - charHeight + 2;
        ctx.drawImage(miyacchiImg, charX, charY, charWidth, charHeight);
    }
